...
repoPath = os.path.join(".", "Burgers-alpha-MLP-PINN")
...
def prep_data(path, N_u=None, N_f=None, N_n=None, q=None, ub=None, lb=None, noise=0.0, idx_t_0=None, idx_t_1=None, N_0=None, N_1=None):
  ...
  #  x = np.load("data/data3_x_new.npy")[:, None]
  #  t = np.load("data/data3_t_new.npy")[:, None]
  #  Exact_u = np.load("data/data3_v_new.npy")
    
    x = np.load("data/data5_x.npy")[:, None]
    t = np.load("data/data5_t.npy")[:, None]
    Exact_u = np.load("data/data5_v.npy")

    # Next 'if' statement is for a discrete method for the identification.
